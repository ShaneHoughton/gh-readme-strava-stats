...
const buildCard = async (
  name: any,
  stravaActivityArr: ActivityEnums[],
  variant = 'original',
) => {
  const calculatedHeight =
    BASE_CARD_HEIGHT + STAT_SECTION_HEIGHT * stravaActivityArr.length;
  ...
  return `<svg width="${CARD_WIDTH + 10}" height="${calculatedHeight + 10}" xmlns="http://www.w3.org/2000/svg">
      ${style}
      ...
      x="${CARD_COL1}"
      y="${TITLE_START}"
      class="card-text card-title" >
      ${name}'s Year-to-Date Strava Stats
      </text>
      ${activityStats}
    </svg>`;
};
